...
internal const val METADATA_LEGACY_ELEMENTS_CONFIGURATION_NAME = "metadataLegacyElements"
...
open class KotlinMetadataTarget(project: Project) : KotlinOnlyTarget<KotlinCommonCompilation>(project, KotlinPlatformType.common) {
    override val kotlinComponents: Set<KotlinTargetComponent> by lazy {
        if (!project.isKotlinGranularMetadataEnabled)
            ...
            usageContexts += run {
                project.configurations.create(METADATA_LEGACY_ELEMENTS_CONFIGURATION_NAME).apply {
                ...
                DefaultKotlinUsageContext(
                    compilations.getByName(KotlinCompilation.MAIN_COMPILATION_NAME),
                    javaApiUsage,
                    METADATA_LEGACY_ELEMENTS_CONFIGURATION_NAME
                )
            }
