...
object Yarn : NpmApi {
    ...
    private fun tryReadYarnLock(
        ...
        if (yarnLock.isFile) {
            ...
            fun resolveRecursivly(src: NpmDependency): NpmDependency {
                if (src in visited) return src
                visited.add(src)

                val key = src.key + "@" + src.version
                val deps = byKey[key]
                if (deps != null) {
                    ...
                    println("[WARN] cannot find $key in yarn.lock")
                }
    ...
    override fun resolveProject(npmPackage: NpmResolver.NpmPackage) {
        ...
        if (!project.yarn.useWorkspaces) {
            ...
            tryReadYarnLock(NpmProjectLayout[project].nodeWorkDir, npmPackage.npmDependencies)
        }
    }
    ...
    override fun resolveRootProject(
        ...
        if (rootProject.yarn.useWorkspaces) {
            ...
            tryReadYarnLock(NpmProjectLayout[rootProject].nodeWorkDir, subprojects.flatMap { it.npmDependencies })
        } else {
